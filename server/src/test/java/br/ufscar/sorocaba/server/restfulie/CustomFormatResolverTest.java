...
public class CustomFormatResolverTest {
	...
	public void extractFormatAndVendorFromHeader() {
		...
		context.checking(new Expectations(){{
			...
			allowing(request).getHeader(with(equal("Accept"))); will(returnValue("application/vnd.ufscar.unit.test+json"));
		...
		assertEquals("json", resolver.getAcceptFormat());
		assertEquals("vnd.ufscar.unit.test", resolver.getVendor());
	}
